module MemoryTests       
        ...
        *)
        ...
        let expectedResult error mems (data:DataPath) point= 
            match error with
            |Some e -> e |> Error
            |None -> (mems,data,data.Regs.Item(point)) |> Ok
        ...
        let mems:MachineMemory<Instruction> = 
                let codeMap =
                    [0u..0x4u..0xFCu]
        ...
        let executionExpectedOutput =
            ...
            //3
             (mems,dataPath,testInstrAccessError),expectedResult (Some CannotAccessCodeMem) mems dataPath R13;
            //4
             (mems,dataPath,testInstrNotDivError),expectedResult (Some WANotDivBy4) mems dataPath R13;
            //5
            ...
            //12
             (mems,dataPath,testInstrAccessError_LDM),expectedResult (Some CannotAccessCodeMem) mems dataPath R13;
            //13
             (mems,dataPath,testInstrNotDivError_LDM),expectedResult (Some WANotDivBy4) mems dataPath R13;
            //14
        ...
        [<EntryPoint>]
            let main _ = 
                ...
                printfn "Now running parsing tests " 
                testParseTopFunction listCornerCases
                printfn "Now running execution tests " 
                testExecutionTopFunction executionExpectedOutput
